...
with open( 'WGUPS_Package_File.csv' ) as csv_file:
    ...
    for row in csv_reader:
        if line_count == 0:
            ...
            line_count += 1
            packageFile.insert( row[0].__str__(), row[1].__str__(), row[5].__str__(), row[2].__str__(),
                                row[4].__str__(),
...
trucks = [1, 4, 5, 7, 8, 9, 10, 11, 13, 14, 15, 16, 17, 19, 20, 21, 2, 3, 12, 18, 22, 24, 26, 27, 29, 30, 31, 33, 36,
          38, 6, 23, 25, 28, 32, 34, 35, 37, 39, 40]

truck1 = [1, 2, 4, 6, 7, 17, 20, 21, 25, 27, 29, 33, 39, 40]  # leaves at 09:05

truck2 = [3, 5, 8, 10, 13, 14, 15, 16, 18, 19, 25, 30, 31, 34, 36, 37, 38]  # leaves at 09:05

truck3 = [9, 11, 12, 22, 23, 24, 26, 28, 32,
          35]  # leaves after truck 1 gets back as there are three trucks, but only 2 drivers
...
def updateAddress(package):
    ...
    for j in range( len( distList ) ):
        if addSplit[0] in distList[j]:
            ...
            packageFile.insert( package.__str__(), distList[j], packageFile.search( package.__str__() )[1],
                                packageFile.search( package.__str__() )[2],
                                addSplit[3],
                    ...
                    if j == value:
                        if key in truck:
                            if key == 25 and checks.twofiveFound is False:
                                ...
                                closest.add( input, shortest )
                                neighborsList.add( input, key )
                                print( 'New shortest distance for package', input, ':', shortest,
                                       'found at index', j,
                                ...
                                if float( newlist[j][0][0] ) < float( shortest ):
                                    ...
                                    closest.add( input, shortest )
                                    neighborsList.add( input, key )
                                    print( 'New shortest distance for package', input, ':', shortest, 'found at index',
                                           j,
                                           'which belongs to package', key, 'at', returnDistAddress( j ) )
    else:
...
def stopOrder(input, truck):
    ...
    for i in closest:
        if closest[i] < quickest and i not in used:
            print( 'Closest neighbor for package', input, 'on the truck: Package', neighborsList.__getitem__( input ),
                   'with a distance of', closest.__getitem__( input ) )
    ...
    if truck == truck1:
        ...
        if input == 0:
            ...
            print( 'Depart time:', stopsa.departTime )
        ...
        if checks.checkall is True:
            ...
            if finalTime > time:
                ...
                print( 'Total mileage for truck 1:', stopsa.mileage )
                print( 'Total mileage for truck 2:', stopsb.mileage )
                print( 'Total mileage for truck 3:', stopsc.mileage )
                print( 'Final total mileage for all trucks:', stopsa.mileage + stopsb.mileage + stopsc.mileage )
                quit()
        ...
        if input == 0:
            ...
            print( 'Depart time:', stopsb.departTime )
...
def returnToHubA(stop):
    ...
    a = stopsa.startTime
    stopsa.finalTime = addMins( a, addedHours, addedMinutes, addedSeconds )
    print( 'Finishing Time: ' + stopsa.finalTime.__str__() )
